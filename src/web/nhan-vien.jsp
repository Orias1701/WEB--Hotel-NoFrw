        ...
        <div style="padding: 10px 25px;">
            ...
            <div class="toolbar"
                ...
                <div class="search-box">
                    ...
                    <input type="text" id="txtSearchNV" class="rounded-input" placeholder="Nhập tên, Email hoặc SĐT..."
                        onkeyup="filterNVTable()" style="width: 250px;">
                    ...
                    <tbody>
                        <c:forEach var="nv" items="${listNhanVien}">
                            ...
                            <tr onclick="openNVForm('update', this.dataset)" data-id="${nv.getMaNhanVien()}"
                                ...
                                <td class="center_cell">${nv.getMaNhanVien()}</td>
                                <td>${nv.getTenNhanVien()}</td>
                                <td>${nv.getSoDienThoai()}</td>
                                <td>${nv.getEmail()}</td>
                                <td class="action-cell center_cell">
                                    <form id="frmDelNV_${nv.getMaNhanVien()}" action="nhan-vien-data" method="post"
                                        style="display:none;">
                                    ...
                                    <button type="button" class="btn-swing btn-danger"
                                        ...
                                        onclick="event.stopPropagation(); confirmDelete('Bạn có chắc chắn muốn xóa nhân viên này?', 'nhanvien', this.closest('tr').dataset.id, 'frmDelNV_' + this.closest('tr').dataset.id);">Xóa</button>
                                </td>
                            </tr>
                        </c:forEach>
                    </tbody>
