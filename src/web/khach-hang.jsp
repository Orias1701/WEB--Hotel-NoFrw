        ...
        <div style="padding: 10px 25px;">
            ...
            <div class="toolbar"
                ...
                <div class="search-box">
                    ...
                    <input type="text" id="txtSearchKH" class="rounded-input" placeholder="Nhập tên, SĐT hoặc Email..."
                        onkeyup="filterKHTable()" style="width: 250px;">
                    ...
                    <tbody>
                        <c:forEach var="kh" items="${listKhachHang}">
                            <tr onclick="openKHForm('update', this.dataset)" data-id="${kh.getMaKhachHang()}"
                                ...
                                <td class="center_cell">${kh.getMaKhachHang()}</td>
                                <td>${kh.getTenKhachHang()}</td>
                                <td>${kh.getSoDienThoai()}</td>
                                <td>${kh.getEmail()}</td>
                                <td class="action-cell center_cell">
                                    <form id="frmDelKH_${kh.getMaKhachHang()}" action="khach-hang-data" method="post"
                                        style="display:none;">
                                    ...
                                    <button type="button" class="btn-swing btn-danger"
                                        ...
                                        onclick="event.stopPropagation(); confirmDelete('Xóa khách hàng này?', 'khachhang', this.closest('tr').dataset.id, 'frmDelKH_' + this.closest('tr').dataset.id);">Xóa</button>
                                </td>
                            </tr>
                        </c:forEach>
                    </tbody>
